const fs = require('fs');
const lines = fs.readFileSync('src/components/MartAdmin.tsx', 'utf8').split('\n');
const startIdx = lines.findIndex(l => l.includes('{history.nao3_sale_items?.length > 0 ? ('));
const endIdx = lines.findIndex((l, i) => i > startIdx && l.includes('<div className="p-4 text-center text-sm text-gray-400">상세 품목 데이터가 없습니다.</div>'));
