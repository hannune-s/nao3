...
const startTarget = `{history.nao3_sale_items?.length > 0 ? (`;
const endTarget = `<div className="p-4 text-center text-sm text-gray-400">상세 품목 데이터가 없습니다.</div>
                    )}`;

const startIndex = c.indexOf(startTarget);
const endIndex = c.indexOf(endTarget) + endTarget.length;
...
if (startIndex !== -1 && endIndex !== -1) {
  ...
  fs.writeFileSync('src/components/MartAdmin.tsx', c);
  console.log('Replaced history item block with pagination.');
} else {
