...
// Find the input block for storeName and append slug input
...
// Here, params.storeId is actually the slug! So we fetch by slug to get the actual store.id
...
adminRoute = adminRoute.replace(/return <ButcherAdmin storeId=\{storeData\.id\} storeName=\{storeData\.store_name\} \/>;/, 
  "return <ButcherAdmin storeId={storeData.id} storeName={storeData.store_name} storeSlug={storeData.slug} />;");
...
martAdmin = martAdmin.replace(/initialStoreName: string;\n\}/, "initialStoreName: string;\n  storeSlug?: string;\n}");
